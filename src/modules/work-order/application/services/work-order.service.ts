...
export class WorkOrderService implements InterfaceWorkOrderUseCase {
  ...
  async createWorkOrder(workOrder: CreateWorkOrderRequest): Promise<WorkOrderResponse | null> {
    try {
      const requiredFields: string[] = ['description', 'workOrderTypeId', 'priorityId', 'workOrderStatusId', 'connectionId', 'createdUserId', 'clientId'];

      const missingFieldMessages: string[] = validateFields(workOrder, requiredFields);
      ...
      const workOrderModel: WorkOrderModel = WorkOrderMapper.fromCreateWorkOrderRequestToWorkOrderModel(workOrder);
      const createWorkOrder = await this.workOrderRepository.createWorkOrder(workOrderModel);
  ...
  async updateWorkOrder(workOrderId: number, workOrder: CreateWorkOrderRequest): Promise<WorkOrderResponse | null> {
    try {
      ...
      const workOrderModel: WorkOrderModel = WorkOrderMapper.fromCreateWorkOrderRequestToWorkOrderModel(workOrder);
      const updateWorkOrder = await this.workOrderRepository.updateWorkOrder(workOrderId, workOrderModel);
  ...
  async getWorkOrderById(workOrderId: number): Promise<WorkOrderResponse | null> {
    try {
      if (!workOrderId || workOrderId <= 0) {
        throw new RpcException({
          statusCode: statusCode.BAD_REQUEST,
      ...
      const workOrder = await this.workOrderRepository.getWorkOrderById(workOrderId);
  ...
  async getWorkOrdersByClientId(clientId: string): Promise<WorkOrderResponse[]> {
    try {
      if (!clientId) {
        throw new RpcException({
          statusCode: statusCode.BAD_REQUEST,
      ...
      const workOrders = await this.workOrderRepository.getWorkOrdersByClientId(clientId);
